...
opts 表示安装时的一些其参数，如当前在开发环境哈还是测试环境，是否在全局环境等。
  */
  ...
  if (!opts) opts = {}
  this.where = where // 当前运行install命令的目录
  this.dryrun = dryrun // true表明你不需要让npm做出任何更新并且只报告完成了什么
  this.args = args // 运行install命令传进来的参数
  ...
  this.currentTree = null // node_moudules 构成的 tree
  this.idealTree = null // 安装相应的模块后 理想的 tree
  this.differences = [] // 两棵树的差异队列
  this.todo = []
  this.progress = {} // 进程管理
  this.noPackageJsonOk = !!args.length // install没有安装对象 也就是arg.length = 0 
  // 此时!!0 为false,就是说没有packageJson是不行的，因为这时候运行的是npm install ,要去拉取package.json里的依赖去安装，所以没有package.json是不行的。
  this.topLevelLifecycles = !args.length  // 顶层元素的生命周期

  this.autoPrune = npm.config.get('package-lock') // 读取配置文件 package-lock 判断是否自动生成package-lock.json文件

  const dev = npm.config.get('dev')
  const only = npm.config.get('only') // dev或者development时，不带任何参数运行局部npm install，
  // 只会有devDependencies(和他们的依赖)会被安装，当是prod或者production是，无参运行npm install，只有non-devDependencies(和他们的依赖)会被安装
  ...
  this.packageLockOnly = opts.packageLockOnly != null
    ? opts.packageLockOnly : npm.config.get('package-lock-only') // 为true，则不进行安装动作，只是将安装的内容写进package.json的dependencies里面
